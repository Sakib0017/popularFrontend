...
export default function Layout() {
  ...
  const navList = (onGo) => (
    <>
      {links.map(([to, l, ic]) => (
        <Link key={to} to={to} onClick={onGo} className={`flex items-center py-3 px-4 rounded-lg transition ${loc.pathname === to ? 'bg-green-700' : 'hover:bg-green-700'}`}>
          <i className={`fas ${ic} mr-3 w-5`}></i>{l}
        </Link>
      ))}
      <Link to="/" onClick={onGo} className="flex items-center py-3 px-4 rounded-lg hover:bg-green-700"><i className="fas fa-globe mr-3 w-5"></i>View Site</Link>
    </>
  );
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <aside className="w-64 bg-[#00984a] text-white flex-col shadow-xl hidden md:flex shrink-0 min-h-screen sticky top-0 h-screen">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-72 max-w-[85vw] bg-[#00984a] text-white flex flex-col shadow-2xl overflow-y-auto">
            <div className="p-5 text-xl font-bold border-b border-green-700 flex items-center justify-between">
              <span><i className="fas fa-hospital-user mr-2"></i>PMC</span>
              <button onClick={() => setMobileOpen(false)} aria-label="Close menu" className="p-2 hover:bg-green-700 rounded-lg"><i className="fas fa-times"></i></button>
            </div>
            <nav className="flex-grow p-4 space-y-1">
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen">
        <header className="bg-white border-b p-3 sm:p-4 flex justify-between items-center gap-3 px-4 md:px-8 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <img className="h-8 w-8 sm:h-9 sm:w-9 rounded-full shrink-0" src={`https://ui-avatars.com/api/?name=${user?.email}&background=00984a&color=fff`} alt="" />
          </div>
        </header>
