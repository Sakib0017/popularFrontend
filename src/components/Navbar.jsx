    ...
    <div className="border-b border-slate-100">
      ...
      {open && (
        <div className="pb-3 pl-2 flex flex-col normal-case tracking-normal font-semibold text-slate-600 text-[13px]">
          {links.map(([to, label]) => (
            <Link key={to} to={to} onClick={close} className="py-2 border-b border-slate-50 last:border-0 hover:text-emerald-600">{label}</Link>
          ))}
        </div>
      )}
    </div>
      ...
      <nav className="bg-white shadow-md sticky top-0 z-[100] border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="flex items-center justify-between h-16 sm:h-20 lg:h-24 gap-3">
            <div className="flex-shrink-0 min-w-0">
              <Link to="/" className="flex items-center gap-2 sm:gap-3" onClick={close}>
                ...
                <span className="hidden min-[420px]:block leading-tight">
                  <span className="block text-[13px] sm:text-sm font-black text-[#003B46] tracking-tight">MEDICAL COLLEGE</span>
                  <span className="block text-[10px] sm:text-[11px] font-semibold text-emerald-700 tracking-widest uppercase">Producing Doctors With a Difference</span>
                </span>
              </Link>
            </div>
            {/* Desktop menu — lg and up (tablet portrait uses drawer) */}
            <div className="hidden lg:flex items-center space-x-4 xl:space-x-5 text-[13px] font-extrabold tracking-tight text-[#003B46]">
              ...
              <div className="group h-24 flex items-center relative">
                ...
                <div className="absolute left-0 top-full w-[290px] shadow-2xl border-t-4 border-emerald-600 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[110] bg-white rounded-b-md">
                  <div className="p-2 flex flex-col max-h-[60vh] overflow-y-auto text-[12px] font-semibold text-slate-700 normal-case">
                    {DEPARTMENTS.map(([s, l]) => <Link key={s} to={`/dept/${s}`} className="p-2 hover:bg-slate-50 hover:text-emerald-600 rounded">{l}</Link>)}
              ...
              <div className="group h-24 flex items-center relative">
                ...
                <div className="absolute left-0 top-full w-[270px] shadow-2xl border-t-4 border-emerald-600 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[110] bg-white rounded-b-md">
                  <div className="p-2 flex flex-col text-[12px] font-semibold text-slate-700 normal-case">
                    {ADMINISTRATION.map(([s, l]) => <Link key={s} to={s} className="p-2 hover:bg-slate-50 hover:text-emerald-600 rounded">{l}</Link>)}
                  </div>
                </div>
              </div>
              <div className="group h-24 flex items-center relative">
                ...
                <div className="absolute left-0 top-full w-[270px] shadow-2xl border-t-4 border-emerald-600 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[110] bg-white rounded-b-md">
                  <div className="p-2 flex flex-col max-h-[60vh] overflow-y-auto text-[12px] font-semibold text-slate-700 normal-case">
                    {FACILITIES.map(([s, l]) => <Link key={s} to={s} className="p-2 hover:bg-slate-50 hover:text-emerald-600 rounded">{l}</Link>)}
                  </div>
                </div>
              </div>
            ...
            <div className="hidden md:flex items-center h-full pl-4 z-20">
              <a href="tel:10606" className="bg-[#007A78] text-white flex flex-col justify-center items-center px-4 lg:px-5 h-[90px] lg:h-[110px] rounded-b-2xl shadow border-x border-b border-white/10 hover:bg-[#006664] -translate-y-[18px] lg:-translate-y-[25px]"><span className="text-[10px] lg:text-[11px] font-bold tracking-widest opacity-90 uppercase">Hotline</span><span className="text-lg lg:text-xl font-black mt-0.5">10606</span></a>
            </div>
            <div className="lg:hidden flex items-center">
              <button onClick={() => setOpen(!open)} aria-label="Toggle menu" aria-expanded={open} className="text-[#003B46] p-2 hover:bg-slate-50 rounded-md">
                {open
        ...
        {open && (
          <div className="lg:hidden bg-white border-t border-slate-100 shadow-inner max-h-[75vh] overflow-y-auto">
            <div className="px-5 sm:px-8 py-4 text-[12px] font-bold tracking-wider uppercase text-[#003B46]">
              ...
              <a href="tel:10606" className="bg-[#007A78] text-white text-center py-3 rounded-lg font-bold text-sm mt-4 mb-2 block">CALL HOTLINE: 10606</a>
            </div>
          </div>
        )}
      </nav>
